package com.ioe.imageclustering.gmm.jmef;

import com.ioe.imageclustering.gmm.jmef.Clustering.CLUSTERING_TYPE;
...
public class BregmanHardClustering {
	...
	private static void computeCentroids(MixtureModel f, MixtureModel g, Clustering.CLUSTERING_TYPE type, int[] repartition){
		...
		for (i=0; i<n; i++){
                    System.out.println("repartition["+i+"]="+repartition[i]);
			count[repartition[i]]++;
                }
		for (i=0; i<m; i++){
