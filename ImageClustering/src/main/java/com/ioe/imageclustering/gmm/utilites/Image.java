...
import com.ioe.imageclustering.common.util.ObjectToByte;
import com.ioe.imageclustering.gmm.jmef.BregmanSoftClustering;
import com.ioe.imageclustering.gmm.jmef.MixtureModel;
import com.ioe.imageclustering.gmm.jmef.MultivariateGaussian;
import com.ioe.imageclustering.gmm.jmef.PVector;
import com.ioe.imageclustering.gmm.jmef.PVectorMatrix;
...
public class Image {
    ...
    public static BufferedImage segmentColorImageFromMOG(BufferedImage imgIn, MixtureModel mm) {
        ...
        // Loop on pixels
        for (int row = 0; row < imgIn.getWidth(); row++) {
            for (int col = 0; col < imgIn.getHeight(); col++) {
    ...
    /**
     * Load a mixture model from a file. If the mixture doesn't exist, the
     * function create a mixture of multivariate Gaussians from the pixels of
     * the image, and save this mixture.
     *
     * @param path file-path of the mixture model
     * @param image input image
     * @param n number of components in the mixture model
     * @return a mixture of Gaussian of n components computed from the input
     * image
     */
    public static MixtureModel loadMixtureModel(String path, BufferedImage image, int n) {
        ...
        if (mm == null) {
            ...
            PVector[] px = convertColorImageToPointSet5D(image);
    ...
    public static byte[] getGMM(BufferedImage image, int n) throws IOException {
        System.out.println("getGMM method is called");
        MixtureModel mm = null;
        System.out.println("gmm is null");
        PVector[] px = convertColorImageToPointSet5D(image);
        if(px==null){
            System.out.println("px is null");
        }System.out.println("px is not null");
        Vector<PVector>[] clusters = KMeans.run(px, n);
        if(clusters==null){
        ...
        if(mm==null){
            System.out.println("mm is null");
        }System.out.println("mm is not null");
        System.out.println("marker starting bregman");
        mm = BregmanSoftClustering.run(px, mm);
        byte[] bytesArray= ObjectToByte.objectToByteArray(mm);
        System.out.println("bytes are"+bytesArray.toString());
        return bytesArray;
